...
* {ACHDirect}[http://www.achdirect.com/] - US
...
== Sample Usage for credit card payments
  require 'rubygems'
...
== Sample Usage for echeck payments
  require 'rubygems'
  require 'active_merchant'
  ...
  echeck = ActiveMerchant::Billing::Check.new(
                  :first_name        	=> 'Bob',
                  :last_name          	=> 'Bobsen',
                  :routing_number     	=> '4242424242424242',
                  :account_number     	=> '12345678',
                  :account_holder_type 	=> 'Business',
                  :account_type 	=> 'Checking'
                )

  billing_address = { :first_name => 'Bob',
		      :last_name => 'Beobsen',
		      :address1 => '1313 Mockingbird Land',
		      :address2 => 'Sweet 1',
		      :city => 'Transylvania',
		      :state => 'PA',
		      :country => 'US',
		      :zip => bank_account.bill_to_zip}



  # Validating the bank routing number
  if echeck.valid_routing_number?
    gateway = ActiveMerchant::Billing::AchDirectGateway.new(:login => $ACHDIRECT_LOGIN, :password => $ACHDIRECT_PASSWORD)

    # Process payment	
    response = gateway.eft_purchase(amount, echeck, :ip=>request.remote_ip, :billing_address=> billing_address)

    if response.success?
      gateway.capture(amount, response.authorization)
      flash[:notice] = "Successfully charged $#{amount} to the bank account #{params[:bank_account][account_number]}"
    else
      raise StandardError, response.message 
    end
  end
